...
\begin{table}[h!]
	...
	\begin{tabular}{ |c|l| }
		...
		SC multi-threaded   & MUZZ, Conzzer, RFF, etc                             \\
		weak multi-threaded & ??                                                  \\
		\hline
	\end{tabular}
...
Consider the store buffer (SB) \ref{SB} example, where x, y are shared variables, and r1, r2 are local variables, all initialized with 0. Under SC, none of the possible thread interleavings (e.g., abcd, acbd, acdb, etc.) result in both r1 and r2 reading the value 1.
