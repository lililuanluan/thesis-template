...
% \[f_{rel}(g) = \frac{f_g^2}{\sum_{g_i \in G} f_{g_i}^2 / |G|}
% \], where $f_{g_i}$ is the number of occurrence of the execution graph $g_i$ for all graphs in the set of explored graphs $G$. For example, if the fuzzer explores $g_1$, $g_2$ and $g_3$ for 2, 3 and 5 times in a total number of 10 explorations, the relative frequency of $g_1$ defined here is $\frac{2^2}{(2^2 + 3^2 + 5^2) / 3} \approx 0.32 $, and for $g_3$ it is $\frac{5^2}{(2^2 + 3^2 + 5^2) / 3} \approx 1.97 $. If the value is lower than a certain threshold, the corresponding execution graph is considered to be interesting. For example, if the threshold is chosen to be 0.4, $g_1$ is considered interesting while $g_3$ is not. 

% Here we provide the intuition of this computation of frequencies. If the is\_interesting function only checks whether a graph is new, each graph can only be identified as interesting once. We use frequencies to provide more granularity than binary feedback. However, using the frequency of a graph alone is not sufficient in some cases. For example, if we chose the threshold for frequencies to be 0.2 in the above example, all execution graphs have higher frequencies than the threshold. In this case, none of the graphs are considered interesting. However, since $g_1$ has a lower frequency than the other graphs, it would be good to prioritize it over others. In other words, the threshold of 0.2 is too low in this case. Consider another case, where the fuzzer finds 100 different graphs $g_1, ..., g_{100}$ with frequencies of 0.01, 0.02, 0.015, ..., each of which is lower than 0.1. In this case all graphs are considered interesting. However, it would be good to only prioritize those graphs with relatively low frequencies than others, instead of regarding all of them intersting. In other words, the threshold is too high for the second case. To solve this problem, we use the relative frequency instead, which not only include the frequency of the judged graph itself, but also include frequencies of other graphs. By squaring each term the differences of frequencies are amplified and it also prevents the sum of frequencies, which is always 1, to be discarded.
...
\]
where \( f_{g_i} \) is the number of occurrences of the execution graph \( g_i \) in the set of explored graphs \( G \). For example, if the fuzzer explores graphs \( g_1 \), \( g_2 \), and \( g_3 \) 2, 3, and 5 times respectively in 10 total explorations, the relative frequency of \( g_1 \) is:
\[
