...
def print_table_with_data(tests, metrics, data):
    ...
    # 打印分隔线
    print("-" * table_width, end="-|-")
    print("-" * ((table_width + 2) * len(tests) + 3 ), end = "-\n")
...
def print_table(file_postfix, is_c11tester = False):
    tests = ["barrier", "chase-lev-deque", "mpmc-queue", "linuxrwlocks", "mcs-lock", ]
    # tests = ["dekker-change", "rwlock-test", "seqlock-test", "bipartite_buf", "left_right", "ring_buf"]
