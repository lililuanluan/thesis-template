...
Secondly, different orders of adding events and forming relations may result in the same executions. For example, consider Listing~\ref{P6}. The test generator could first select \texttt{r1} and set its write to read from as \texttt{s0}, or it could first add \texttt{s1-3} and then set \texttt{r1} to read from \texttt{s0}. Both cases can result in equivalent executions.
...
\begin{figure}[h!tbp] % htbp 表示优先放置位置：here, top, bottom, page
    \centering
    \includegraphics[scale=0.8]{figure/exec-graph/SB4.pdf} % 图片文件名和路径  
