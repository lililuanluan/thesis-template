...
\begin{lstlisting}[caption={fuzzing example}, label={exp-fuzz}]
    ...
    void thread2() {
        auto r1 = var.load(acquire);    // r1
        auto r2 = var.load(acquire);    // r2
    }
    \end{lstlisting}
...
\begin{figure}[htbp] % htbp 表示优先放置位置：here, top, bottom, page
    \centering
    \includegraphics[scale=0.5]{figure/example_construct.pdf} % 图片文件名和路径  
    \caption{construction of an execution graph} % 图片标题
    \label{example_construct} % 图片标签，用于交叉引用
\end{figure}

\begin{figure}[htbp] % htbp 表示优先放置位置：here, top, bottom, page
    \centering
    \includegraphics[scale=0.5]{figure/example_mutate.pdf} % 图片文件名和路径  
    \caption{mutate the previous execution graph and re-explore} % 图片标题
    \label{example_mutate} % 图片标签，用于交叉引用
\end{figure}
