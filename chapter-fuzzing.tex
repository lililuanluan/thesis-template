...
In addition, the probability of encountering different executions is not uniformly distributed. Some execution graphs are easier to find, while others may have a lower probability of being discovered. Consider another example, shown in Listing~\ref{P2}:
...
Different orders of adding events and forming relations may result in same executions. Take Listing~\ref{P6} for example. The model checker can first select \texttt{r1} first and set its \texttt{rf} as \texttt{s0}, or it can first add \texttt{s1-3} and then set \texttt{r1} reading from \texttt{s0}. Both cases are equivalent.
